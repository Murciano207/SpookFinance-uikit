...
import { ColumnByNamesType, ColumnType, TableState, TableAction, DataType, UseTableReturnType, UseTableOptionsType, RowType, HeaderType, HeaderRenderType, ColumnStateType } from './types';
import { byTextAscending, byTextDescending } from './utils';

const sortByColumn = <T extends DataType>(data: RowType<T>[], sortColumn: string, columns: ColumnStateType<T>[]): RowType<T>[] => {
...
export const useTable = <T extends DataType>(columns: ColumnType<T>[], data: T[], options?: UseTableOptionsType<T>): UseTableReturnType<T> => {
  ...
  return {
    ...
    toggleSort: (columnName: string, isAscOverride?: boolean) => dispatch({ type: 'TOGGLE_SORT', columnName, isAscOverride }),
